...
coef = np.random.rand(bases, 1)

for i in range(10):
    coef = coef - transpose(
        mul(first_order(design_matrix, Y, coef), inverse(mul(transpose(design_matrix), design_matrix))))

result = mul(mul(inverse(mul(transpose(design_matrix), design_matrix)), transpose(design_matrix)), Y)
# print(result)
print('Newton method:')
...
plt.scatter(X, Y, c='r')
plt.plot(x, polycoefficient(x, coef), 'b-')
plt.title('Using Newton method')
plt.show()
